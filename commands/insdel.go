...
func (c *DeleteWord) Run(v *lime.View, e *lime.Edit) error {
	var class int
	if c.Forward {
		class = lime.CLASS_WORD_END | lime.CLASS_PUNCTUATION_END | lime.CLASS_LINE_START
	} else {
		class = lime.CLASS_WORD_START | lime.CLASS_PUNCTUATION_START | lime.CLASS_LINE_END | lime.CLASS_LINE_START
	}
	...
	sel.AddAll(rs)
	if c.Forward {
		lime.GetEditor().CommandHandler().RunTextCommand(v, "right_delete", nil)
	} else {
		lime.GetEditor().CommandHandler().RunTextCommand(v, "left_delete", nil)
	}
	return nil
}

func (c *DeleteWord) findByClass(point int, class int, v *lime.View) int {
	var end, d int
	if c.Forward {
		d = 1
		end = v.Size()
		...
		if strings.Contains(s, "\t") && strings.Contains(s, " ") {
			class = lime.CLASS_WORD_START | lime.CLASS_PUNCTUATION_START | lime.CLASS_LINE_END
		}
	} else {
		d = -1
		end = 0
