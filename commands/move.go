...
func moveAction(v *lime.View, extend bool, transform func(r text.Region) int) {
	sel := v.Sel()
	r := sel.Regions()
	bs := v.Size()
	for i := range r {
...
func (c *Move) Run(v *lime.View, e *lime.Edit) error {
	...
	case Stops:
		moveAction(v, c.Extend, func(in text.Region) int {
			tmp := v.Settings().String("word_separators", lime.DEFAULT_SEPARATORS)
			defer v.Settings().Set("word_separators", tmp)
			v.Settings().Set("word_separators", c.Separators)

			classes := 0
			if c.WordBegin {
				classes |= lime.CLASS_WORD_START
			}
			if c.WordEnd {
				classes |= lime.CLASS_WORD_END
			}
			if c.PunctBegin {
				classes |= lime.CLASS_PUNCTUATION_START
			}
			if c.PunctEnd {
				classes |= lime.CLASS_PUNCTUATION_END
			}
			if c.EmptyLine {
				classes |= lime.CLASS_EMPTY_LINE
			}
	...
	case Words:
		moveAction(v, c.Extend, func(in text.Region) int {
			return v.FindByClass(in.B, c.Forward, lime.CLASS_WORD_START|
				lime.CLASS_LINE_END|lime.CLASS_LINE_START)
		})
	case WordEnds:
		moveAction(v, c.Extend, func(in text.Region) int {
			return v.FindByClass(in.B, c.Forward, lime.CLASS_WORD_END|
				lime.CLASS_LINE_END|lime.CLASS_LINE_START)
		})
	case SubWords:
		moveAction(v, c.Extend, func(in text.Region) int {
			return v.FindByClass(in.B, c.Forward, lime.CLASS_SUB_WORD_START|
				lime.CLASS_WORD_START|lime.CLASS_PUNCTUATION_START|lime.CLASS_LINE_END|
				lime.CLASS_LINE_START)
		})
	case SubWordEnds:
		moveAction(v, c.Extend, func(in text.Region) int {
			return v.FindByClass(in.B, c.Forward, lime.CLASS_SUB_WORD_END|
				lime.CLASS_WORD_END|lime.CLASS_PUNCTUATION_END|lime.CLASS_LINE_END|
				lime.CLASS_LINE_START)
		})
	case Pages:
...
func (c *Move) Default(key string) interface{} {
	if key == "separators" {
		return lime.DEFAULT_SEPARATORS
	}
	return nil
}
