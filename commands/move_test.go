...
func runMoveTest(tests []MoveTest, t *testing.T, text string) {
	...
	for i, test := range tests {
		...
		args := lime.Args{"by": test.by, "extend": test.extend, "forward": test.forward}
		if test.args != nil {
...
func TestMoveByStops(t *testing.T) {
	...
	tests := []MoveTest{
		...
		},
		{
			...
			true,
			[]text.Region{{46, 46}},
			lime.Args{"word_end": true, "separators": ""},
		},
		{
			[]text.Region{{8, 8}},
			"stops",
		...
		},
		{
			...
			true,
			[]text.Region{{4, 4}},
			lime.Args{"word_begin": true, "separators": "l"},
		},
		{
			[]text.Region{{58, 58}},
			"stops",
...
func (f *scfe) VisibleRegion(v *lime.View) text.Region {
	s := v.Line(v.TextPoint(3*3, 1))
	e := v.Line(v.TextPoint(6*3, 1))
	return text.Region{s.Begin(), e.End()}
}
func (f *scfe) Show(v *lime.View, r text.Region) {
	f.show = r
}
...
func TestScrollLines(t *testing.T) {
	...
	v.EndEdit(e)
	ch.RunTextCommand(v, "scroll_lines", lime.Args{"amount": 0})

	if c := v.Line(v.TextPoint(3*3, 1)); fe.show.Begin() != c.Begin() {
		t.Errorf("Expected %v, but got %v", c, fe.show)
	}

	ch.RunTextCommand(v, "scroll_lines", lime.Args{"amount": 1})
