...
func TestRowColLineWord(t *testing.T) {
	...
	d, err := ioutil.ReadFile("./testdata/unittest.json")
	if err != nil {
		t.Fatal(err)
	}
	...
	d, err = ioutil.ReadFile("./testdata/unittest.cpp")
	if err != nil {
		t.Fatal(err)
	}
...
func fill(data []rune) {
	...
	var (
		wordSize, lineSize int
	)

	for i := range data {
		if wordSize <= 0 {
			wordSize = minWord + rand.Intn(maxWord-minWord)
			data[i] = ' '
		} else if lineSize <= 0 {
			lineSize = minLine + rand.Intn(maxLine-minLine)
			data[i] = '\n'
		} else {
