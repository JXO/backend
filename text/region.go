...
// Region defines a Region from point A to B.
// A can be less than B, in which case the selection is inverted.
type Region struct {
	A, B int
}
...
// Begin returns the starting point of the region,
// that would be whichever of A and B is the minimal value.
func (r Region) Begin() int {
	return Min(r.A, r.B)
}

// End returns the ending point of the region,
// that would be whichever of A and B is the maximum value.
func (r Region) End() int {
	return Max(r.A, r.B)
}

// Contains returns whether the region contains the given
// point or not.
...
// Clip clips this Region against the Region provided in the argument.
...
// Cut cuts away the parts of the region that is in the argument region.
