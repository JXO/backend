...
func (o *Settings) Py_get(tu *py.Tuple, kw *py.Dict) (py.Object, error) {
	...
	if v2, ok := v.(*py.Unicode); !ok {
		return nil, fmt.Errorf("Expected type *py.Unicode for lime.Settings.Get() arg1, not %s", v.Type())
	} else {
		arg1 = v2.String()
	}
...
func (o *Settings) Py_set(tu *py.Tuple, kw *py.Dict) (py.Object, error) {
	...
	if v2, ok := v.(*py.Unicode); !ok {
		return nil, fmt.Errorf("Expected type *py.Unicode for lime.Settings.Set() arg1, not %s", v.Type())
	} else {
		arg1 = v2.String()
	}
	v, err = tu.GetItem(1)
