...
func (o *Window) Py_open_file(tu *py.Tuple) (py.Object, error) {
	...
	if v2, ok := v.(*py.Unicode); !ok {
		return nil, fmt.Errorf("Expected type *py.Unicode for lime.Window.OpenFile() arg1, not %s", v.Type())
	} else {
		arg1 = v2.String()
	}
	if tu.Size() > 1 {
		...
		if v2, ok := v.(*py.Long); !ok {
			return nil, fmt.Errorf("Expected type *py.Long for lime.Window.OpenFile() arg2, not %s", v.Type())
		} else {
			arg2 = int(v2.Int64())
		}
	}
...
func (o *Window) Py_run_command(tu *py.Tuple) (py.Object, error) {
	...
	if v2, ok := v.(*py.Unicode); !ok {
		return nil, fmt.Errorf("Expected type *py.Unicode for lime.Window.RunCommand() arg1, not %s", v.Type())
	} else {
		arg1 = v2.String()
	}
	arg2 = make(lime.Args)
	if v, err := tu.GetItem(1); err == nil {
		v2, ok := v.(*py.Dict)
		if !ok {
			return nil, fmt.Errorf("Expected type *py.Dict for lime.Window.RunCommand() arg2, not %s", v.Type())
		}
	...
	lime.GetEditor().CommandHandler().RunWindowCommand(o.data, arg1, arg2)
	return toPython(nil)
}
...
func (o *Window) Py_focus_view(tu *py.Tuple) (py.Object, error) {
	...
	if v, err := tu.GetItem(0); err != nil {
		...
		if v3, err2 := fromPython(v); err2 != nil {
			...
			if v2, ok := v3.(*lime.View); !ok {
				return nil, fmt.Errorf("Expected type *lime.View for lime.Window.SetActiveView() arg1, not %s", v.Type())
			} else {
				arg1 = v2
			}
		}
